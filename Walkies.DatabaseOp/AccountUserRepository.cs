...
namespace Walkies.DatabaseOperations
{
    ...
    public async Task<AccountUser> GetByUnlockedandEmail(AccountUser accountUser)
    {
        var result = await QueryAsync<AccountUser>(_qs.AccountUser.GetByUnlockedandEmail, accountUser);
        return result.FirstOrDefault();
    }
